...
interface EmailExpenseDialogProps {
  visible: boolean;
  onDismiss: () => void;
...
export const EmailExpenseDialog: React.FC<EmailExpenseDialogProps> = ({
  visible,
  onDismiss,
  onSend,
  expenseDescription,
  ...
  const [subject, setSubject] = useState(`Expense Report - ${expenseDescription}`);
  const [message, setMessage] = useState(`Please find attached expense report for ${expenseDescription}.`);
  ...
  useEffect(() => {
    ...
    setCc(senderEmail || '');
    setSubject(`Expense Report - ${expenseDescription}`);
    setMessage(`Please find attached expense report for ${expenseDescription}.`);
  }, [recipientEmail, senderEmail, expenseDescription]);
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await api.email.preview({
        type: 'expense',
        invoiceNumber: expenseNumber || 'EXP-001',
  ...
  return (
    <Dialog visible={visible} onDismiss={onDismiss} style={styles.dialog}>
      <Dialog.Title>Send Expense Report via Email</Dialog.Title>
      <Dialog.ScrollArea>
        <ScrollView>
            ...
            <View style={styles.recipientsDisplay}>
              ...
              <View style={styles.recipientRow}>
                ...
                <View style={styles.recipientInfo}>
                  ...
                  <Text style={styles.recipientEmail}>{to || 'recipient@example.com'}</Text>
                </View>
              </View>
              ...
              {!!cc && (
                <View style={styles.recipientRow}>
                  ...
                  <View style={styles.recipientInfo}>
                    <MaterialIcons name="business" size={18} color={colors.secondary.main} />
