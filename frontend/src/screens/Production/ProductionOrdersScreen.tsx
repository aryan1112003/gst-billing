...
export const ProductionOrdersScreen: React.FC = () => {
    ...
    const [selectedRecord, setSelectedRecord] = useState<ProductionOrder | null>(null);

    const fetchRecords = useCallback(async (pageNum = 1, search = '') => {
        try {
            const response = await api.productionOrders.getAll({
                page: pageNum,
                limit: 10,
                search,
            });
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.productionOrders.delete(selectedRecord.id.toString());
    ...
    const renderCard = ({ item }: { item: ProductionOrder }) => (
        <Card
            style={[styles.card, { backgroundColor: themeColors.surface.card }]}
            onPress={() => navigation.navigate('ProductionOrderForm' as any, { productionOrderId: item.id } as any)}
        >
            <Card.Content>
                ...
                <View style={styles.cardBody}>
                    <View style={styles.row}>
                        ...
                        <Text variant="bodySmall" style={{ color: themeColors.text.secondary }}>
                            Planned: {item.plannedDate ? new Date(item.plannedDate).toLocaleDateString() : 'N/A'}
                        </Text>
                    </View>
                </View>
        ...
        <MainLayout currentRoute="ProductionOrders" title="Production Orders" onNavigate={handleNavigate}>
            <View style={[styles.container, { backgroundColor: themeColors.background.main }]}>
                        ...
                        <FlatList
                            ...
                            ListEmptyComponent={
                                <View style={styles.emptyContainer}>
                                    ...
                                    <Button
                                        mode="contained"
                                        onPress={() => navigation.navigate('ProductionOrderForm' as any)}
                ...
                <Portal>
                    ...
                    >
                        <Dialog.Title style={{ color: themeColors.text.primary }}>Delete Production Order</Dialog.Title>
                        <Dialog.Content>
                            <Paragraph style={{ color: themeColors.text.secondary }}>
                                Are you sure you want to delete {selectedRecord?.orderNumber}?
                            </Paragraph>
                        </Dialog.Content>
                        <Dialog.Actions>
