...
const getFrequencyColor = (frequency: string) => {
    ...
    return colors[frequency] || '#9E9E9E';
};

export const RecurringInvoicesScreen: React.FC = () => {
    ...
    const [selectedRecord, setSelectedRecord] = useState<RecurringInvoice | null>(null);

    const fetchRecords = useCallback(async (pageNum = 1, search = '') => {
        try {
            const response = await api.recurringInvoices.getAll({
                page: pageNum,
                limit: 10,
                search,
            });
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.recurringInvoices.delete(selectedRecord.id.toString());
    ...
    const renderCard = ({ item }: { item: RecurringInvoice }) => (
        <Card
            style={[styles.card, { backgroundColor: themeColors.surface.card }]}
            onPress={() => navigation.navigate('RecurringInvoiceForm' as any, { recurringInvoiceId: item.id } as any)}
        >
            <Card.Content>
                <View style={styles.cardHeader}>
                    ...
                    <View style={styles.headerRight}>
                        <Chip
                            mode="flat"
                            style={{ backgroundColor: getFrequencyColor(item.frequency), marginBottom: 4 }}
                ...
                <View style={styles.cardBody}>
                    <View style={styles.row}>
                        <Text variant="bodySmall" style={{ color: themeColors.text.secondary }}>
                            Next: {item.nextDate ? new Date(item.nextDate).toLocaleDateString() : 'N/A'}
                        </Text>
        ...
        <MainLayout currentRoute="RecurringInvoices" title="Recurring Invoices" onNavigate={handleNavigate}>
            <View style={[styles.container, { backgroundColor: themeColors.background.main }]}>
                        ...
                        <FlatList
                            ...
                            ListEmptyComponent={
                                <View style={styles.emptyContainer}>
                                    ...
                                    <Button
                                        mode="contained"
                                        onPress={() => navigation.navigate('RecurringInvoiceForm' as any)}
                ...
                <Portal>
                    ...
                    >
                        <Dialog.Title style={{ color: themeColors.text.primary }}>Delete Recurring Invoice</Dialog.Title>
                        <Dialog.Content>
                            <Paragraph style={{ color: themeColors.text.secondary }}>
                                Are you sure you want to delete {selectedRecord?.recurringNumber}?
                            </Paragraph>
                        </Dialog.Content>
                        <Dialog.Actions>
