...
export const BillOfMaterialsScreen: React.FC = () => {
    const navigation = useNavigation() as any;
    const { colors: themeColors } = useTheme();
    const [records, setRecords] = useState<BillOfMaterial[]>([]);
    ...
    const [selectedRecord, setSelectedRecord] = useState<BillOfMaterial | null>(null);

    const fetchRecords = useCallback(async (pageNum = 1, search = '') => {
        try {
            const response = await api.billOfMaterials.getAll({
                page: pageNum,
                limit: 10,
                search,
            });
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.billOfMaterials.delete(selectedRecord.id.toString());
            setRecords(prev => prev.filter(r => r.id !== selectedRecord.id));
            setDeleteDialogVisible(false);
            setSelectedRecord(null);
        } catch (error) {
            console.error('Failed to delete bill of materials:', error);
        }
    };
    ...
    const renderCard = ({ item }: { item: BillOfMaterial }) => (
        <Card
            style={[styles.card, { backgroundColor: themeColors.surface.card }]}
            onPress={() => navigation.navigate('BOMForm' as any, { bomId: item.id } as any)}
        >
            <Card.Content>
        ...
        <MainLayout currentRoute="BillOfMaterials" title="Bill of Materials" onNavigate={handleNavigate}>
            <View style={[styles.container, { backgroundColor: themeColors.background.main }]}>
                        ...
                        <FlatList
                            ...
                            ListEmptyComponent={
                                <View style={styles.emptyContainer}>
                                    ...
                                    <Button
                                        mode="contained"
                                        onPress={() => navigation.navigate('BOMForm' as any)}
                ...
                <Portal>
                    ...
                    >
                        <Dialog.Title style={{ color: themeColors.text.primary }}>Delete Bill of Materials</Dialog.Title>
                        <Dialog.Content>
                            <Paragraph style={{ color: themeColors.text.secondary }}>
                                Are you sure you want to delete {selectedRecord?.bomNumber}?
                            </Paragraph>
                        </Dialog.Content>
                        <Dialog.Actions>
